...
1. Start the server locally in preview-only mode.
2. Run `smoke_test.py`.
3. Confirm `/describe` exposes the expected tools and prompt.
4. Add `OPENAI_API_KEY` and test a bounded read-only task.
5. Enable `AGORAGENTIC_RUN_LIVE=1` only for intentionally scoped live work.
